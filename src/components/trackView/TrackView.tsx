...
import { trackViewArtist, trackViewDetails, trackViewInfo, trackViewTag, trackViewTagsWrapper, trackViewTitle, trackViewWaveformWrapper, trackViewWrapper } from "./TrackView.css"
    ...
    <main>
      <div className={trackViewWrapper}>
        <div className={trackViewInfo}>
          <div>
            <h1 className={trackViewTitle}>
            {track.title}
            </h1>
            <p>by <Link to={`/${track.artist}`} className={trackViewArtist}>{track.artist}</Link></p>
            </div>
          ...
          {track.tags.split(',').map((tag) => (
            <span key={tag} className={trackViewTag}>
              {tag}
            </span>
          ))}
          </div>
